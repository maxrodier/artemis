...
public class TextElement extends RenderableNode {
    ...
    public TextElement(VulkanContext context, SceneRenderer sceneRenderer, String text, int size, Font font) {
        ...
        this.mesh = createMesh(context, text, size, font);
        this.font = font;

        updateDescriptorSets(context.getDevice());
        recordDrawCommandBuffer(context.getDevice(), this.drawCommandBuffer, this.shaderProgram.getGraphicsPipeline(), sceneRenderer.getSceneFramebufferObject().getRenderPass(), sceneRenderer.getSceneFramebufferObject().getFramebuffer(), this.descriptorSets, this.mesh);
    }
    ...
    public void destroy(VulkanContext context, SceneRenderer sceneRenderer) {
        ...
        this.mesh.destroy(context.getMemoryAllocator());
    ...
    public static void recordDrawCommandBuffer(VulkanDevice device, SecondaryCommandBuffer drawCommandBuffer, GraphicsPipeline graphicsPipeline, RenderPass renderPass, VulkanFramebuffer framebuffer, DescriptorSet[] descriptorSets, Mesh mesh) {
    ...
    private static Mesh createMesh(VulkanContext context, String text, int size, Font font) {
        ...
        return new Mesh(context, vertices.toArray(new Vertex[vertices.size()]), indices.toArray(new Integer[indices.size()]), VertexKind.POS_COLOUR_UV);
    }
}
