package ca.artemis.engine.api.vulkan.framebuffer;
...
public class SurfaceSupportDetails {
    ...
    public SurfaceSupportDetails(VulkanPhysicalDevice physicalDevice, VulkanSurface surface, GLFWWindow window) {
        ...
        this.surfaceExtent = chooseSurfaceExtent(window.getHandle(), surfaceCapabilities);
    ...
    private static VkSurfaceCapabilitiesKHR fetchSurfaceCapabilities(VulkanPhysicalDevice physicalDevice, VulkanSurface surface) {
        ...
        int result = KHRSurface.vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice.getHandle(), surface.getHandle(), surfaceCapabilities);
        if(result != VK11.VK_SUCCESS) {
            throw new RuntimeException("Failed to get surface capabilities: " + result);
        }
        return surfaceCapabilities;
    }
    ...
    private static VkSurfaceFormatKHR.Buffer fetchSurfaceFormats(VulkanPhysicalDevice physicalDevice, VulkanSurface surface) {
        try(MemoryStack stack = MemoryStack.stackPush()) {
            ...
            int result = KHRSurface.vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice.getHandle(), surface.getHandle(), pSurfaceFormatCount, null);
            if(result != VK11.VK_SUCCESS) {
                throw new RuntimeException("Failed to get surface formats count: " + result);
            }

            VkSurfaceFormatKHR.Buffer surfaceFormats = VkSurfaceFormatKHR.calloc(pSurfaceFormatCount.get(0));
            result = KHRSurface.vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice.getHandle(), surface.getHandle(), pSurfaceFormatCount, surfaceFormats);
            if(result != VK11.VK_SUCCESS) {
                throw new RuntimeException("Failed to get surface formats: " + result);
            }
    ...
    private static IntBuffer fetchSurfacePresentModes(VulkanPhysicalDevice physicalDevice, VulkanSurface surface) {
        try(MemoryStack stack = MemoryStack.stackPush()) {
            ...
            int result = KHRSurface.vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice.getHandle(), surface.getHandle(), pSurfacePresentModesCount, null);
            if(result != VK11.VK_SUCCESS) {
                throw new RuntimeException("Failed to get surface present modes count: " + result);
            }
            ...
            result = KHRSurface.vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice.getHandle(), surface.getHandle(), pSurfacePresentModesCount, surfacePresentModes);
            if(result != VK11.VK_SUCCESS) {
                throw new RuntimeException("Failed to get surface present modes: " + result);
            }
