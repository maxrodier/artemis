...
public class SpriteNode extends RenderableNode {
    ...
    public SpriteNode(SceneRenderer sceneRenderer, int x, int y, int width, int height, Sprite sprite) {
        super(sceneRenderer.getSpriteShaderProgram(), sceneRenderer.getCommandPool());
        ...
        this.quad = new Quad(x, y, width, height, new Vector3f(1,0,1), sprite.getU(), sprite.getV());
    ...
    public void updateDescriptorSets() {
        ...
        descriptorSets[0].updateDescriptorImageBuffer(sprite.getTexture().getImageBundle().getImageView(), sprite.getTexture().getSampler(), VK11.VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 1, VK11.VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    }
