...
public class VulkanTexture {
    ...
    public VulkanTexture(VulkanContext context, CommandPool commandPool, String filePath, boolean multiSampling) {
        this.imageBundle = createTexutureImageBundle(context.getDevice(), context.getMemoryAllocator(), commandPool, filePath, multiSampling);
    ...
    private static VulkanImage createTextureImage(VulkanDevice device, VulkanMemoryAllocator memoryAllocator, CommandPool commandPool, String filePath, boolean multiSampling) {
        try(MemoryStack stack = MemoryStack.stackPush()) {
            BufferedImage bufferedImage = ImageIO.read(ClassLoader.getSystemResourceAsStream(filePath));
            ...
            CommandBufferUtils.copyBufferToImage(device, device.getGraphicsQueue(), commandPool, stagingBuffer, textureImage, textureImage.getWidth(), textureImage.getHeight());
            if(multiSampling) {
                CommandBufferUtils.generateMipmaps(device, device.getGraphicsQueue(), commandPool, textureImage, textureImage.getWidth(), textureImage.getHeight(), mipLevels);
            } else {
