...
public class VulkanSurface {
    ...
    private long createHandle(VulkanInstance instance, Window window) {
        try(MemoryStack stack = MemoryStack.stackPush()) {
            LongBuffer pSurface = stack.callocLong(1);
            if(GLFWVulkan.glfwCreateWindowSurface(instance.getHandle(), window.getId(), null, pSurface) != VK11.VK_SUCCESS) {
